 ...
 *                    Jerome Fisher;
 *
 * $Id: CandlestickRenderer.java,v 1.7.2.5 2007/03/05 33 mungady Exp $
 *
 * Changes
 * -------
 ...
 *
 * <p>This renderer does not include code to calculate the crosshair point for the plot.
 */
public class CustomCandlestickRenderer extends AbstractXYItemRenderer
  ...
  /**
   * A flag that controls whether or not the renderer's outline paint is used to draw the outline of
   * the candlestick. The default value is <code>false</code> to avoid a change of behaviour for
   * existing code.
   *
   * @since 1.0.5
   */
  ...
  // Custom Colors
  private Color downPaintOutline;
  private Color upPaintOutline;
   ...
   * Creates a new renderer for candlestick charts.
   *
   * <p>Use -1 for the candle width if you prefer the width to be calculated automatically.
   *
   * @param candleWidth The candle width.
   */
  public CustomCandlestickRenderer(double candleWidth) {
   ...
   */
  public CustomCandlestickRenderer(
      double candleWidth, boolean drawVolume, XYToolTipGenerator toolTipGenerator) {
    super();
    setToolTipGenerator(toolTipGenerator);
    this.candleWidth = candleWidth;
    this.drawVolume = drawVolume;
    this.upPaint = Color.green;
    this.upPaintOutline = Color.green.darker();
    this.downPaint = Color.red;
    this.downPaintOutline = Color.red.darker();
   ...
   *
   * <p>If you set the width to a negative value, the renderer will calculate the candle width
   * automatically based on the space available on the chart.
   *
   ...
   *
   * <p><code>WIDTHMETHOD_AVERAGE</code>: Divides the entire display (ignoring scale factor) by the
   * number of items, and uses this as the available width.<br>
   * <code>WIDTHMETHOD_SMALLEST</code>: Checks the interval between each item, and uses the smallest
   * as the available width.<br>
   * <code>WIDTHMETHOD_INTERVALDATA</code>: Assumes that the dataset supports the IntervalXYDataset
   * interface, and uses the startXValue - endXValue as the available width. <br>
   *
  ...
  /**
   * Returns the factor by which the available space automatically calculated for the candles will
  ...
  /**
   * Sets the factor by which the available space automatically calculated for the candles will be
   * multiplied to determine the actual width to use.
   *
  ...
  /**
   * Returns the amount of space to leave on the left and right of each candle when automatically
   * calculating widths.
   *
   * @return The gap.
   * @see #setAutoWidthGap(double)
  ...
  /**
   * Returns the flag that controls whether or not the renderer's outline paint is used to draw the
   * candlestick outline. The default value is <code>false</code>.
   *
   * @return A boolean.
   * @since 1.0.5
  ...
  /**
   * Sets the flag that controls whether or not the renderer's outline paint is used to draw the
   * candlestick outline, and sends a {@link RendererChangeEvent} to all registered listeners.
   *
  ...
  /**
   * Initialises the renderer then returns the number of 'passes' through the data that the renderer
   * will require (usually just one). This method will be called before the first item is rendered,
  ...
  public void drawItem(
      ...
      switch (this.autoWidthMethod) {
        ...
        case WIDTHMETHOD_SMALLEST:
          ...
          for (int i = 0; i < itemCount; i++) {
            double pos =
                domainAxis.valueToJava2D(highLowData.getXValue(series, i), dataArea, domainEdge);
            if (lastPos != -1) {
        ...
        case WIDTHMETHOD_INTERVALDATA:
          ...
          double startPos =
              domainAxis.valueToJava2D(
                  intervalXYData.getStartXValue(series, item), dataArea, plot.getDomainAxisEdge());
          double endPos =
              domainAxis.valueToJava2D(
                  intervalXYData.getEndXValue(series, item), dataArea, plot.getDomainAxisEdge());
    ...
    if (this.drawVolume) {
      ...
      if (horiz) {
        ...
        g2.fill(
            new Rectangle2D.Double(xx - volumeWidth / 2, max - zzVolume, volumeWidth, zzVolume));
      }
    ...
    Shape body = null;
    Shape outline = null;
    double x2 = xx - stickWidth / 2;
    if (horiz) {
      body =
          new Rectangle2D.Double(yyMinOpenClose, x2, yyMaxOpenClose - yyMinOpenClose, stickWidth);
    } else {
      body =
          new Rectangle2D.Double(x2, yyMinOpenClose, stickWidth, yyMaxOpenClose - yyMinOpenClose);
      outline =
          new Rectangle2D.Double(
              x2 + 2, yyMinOpenClose - 2.5, stickWidth + 2, yyMaxOpenClose - yyMinOpenClose);
    }
    if (yClose > yOpen) {
      if (this.upPaint != null) {
        g2.setPaint(this.upPaintOutline);
        g2.fill(outline);
        g2.fillPolygon(
            new int[] {(int) x2, (int) (x2 + 2), (int) (x2 + 2)},
            new int[] {(int) yyMinOpenClose, (int) (yyMinOpenClose - 2.5), (int) yyMinOpenClose},
            3);
/*        g2.fillPolygon(
            new int[] {(int)x2 + stickWidth , 
            yPoints, 
            nPoints);*/
      ...
      if (this.downPaint != null) {
        g2.setPaint(this.downPaintOutline);
        g2.fill(outline);
        g2.fillPolygon(
            new int[] {(int) x2, (int) (x2 + 2), (int) (x2 + 2)},
            new int[] {(int) yyMinOpenClose, (int) (yyMinOpenClose - 2.5), (int) yyMinOpenClose},

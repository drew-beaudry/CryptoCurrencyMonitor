...
/**Handles dropping Chart objects into OptionPaneView*/
public class ChartDropAdapter extends DropTargetAdapter {

  private final Text text;
  private Display display = Display.getCurrent();
  private static Service service = Activator.getService();

  public ChartDropAdapter(Text text) {
    this.text = text;
  }
  ...
  public void drop(DropTargetEvent event) {
    text.setText(((Chart) event.data).getName());
    text.setBackground(display.getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
    service.setChart((Chart) event.data);
    super.drop(event);
  }

...
public class ImageHandler {
  ...
  public Image getImage(Coin coin){
    //If we have already processed resizing for this image,
    //find the resized image and return it
    if (coin.getImage() != null){
      return coin.getImage();
    }
    Display display = Display.getCurrent();
      try {
        //SOON MOVE MARSHALLING OUT OF PLATFORM
        Request request = new Request.Builder()
            .url(Constants.BASEURL + coin.getImgurl())
            .build();
        ...
        //Resize the image
        Image resizeImage = resize(image, Constants.COINICONSIZE);
        
        //Save image to coin so we don't have to process resizing again for this coin
        coin.setImage(resizeImage);
        return resizeImage;
      } catch (IOException e) {
        e.printStackTrace();
      }
